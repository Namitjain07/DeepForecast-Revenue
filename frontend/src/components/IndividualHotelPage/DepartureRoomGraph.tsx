...
const DepartureRoomGraph: React.FC<DepartureRoomGraphProps> = ({ hotelId }) => {
    ...
    const { departure: departureRecords } = useSelector((state: RootState) => state.records);
    const { departure: departureForecasts } = useSelector((state: RootState) => state.forecast);
    ...
    return (
        <div className="component-ui-departure-room-graph-container">
            <div className="component-ui-departure-room-graph-header">
                <h3 className="component-ui-departure-room-graph-title">Departure Room Analysis</h3>
                <div className="component-ui-departure-room-graph-toggle">
                    {(['1w', '1m', '3m', '6m', '12m'] as const).map(period => (
                        <button
                            key={period}
                            className={`component-ui-departure-room-graph-toggle-btn ${timePeriod === period ? 'active' : ''}`}
            ...
            <ResponsiveContainer width="100%" height={chartHeight}>
                {chartType === 'bar' ? (
                    ...
                    >
                        ...
                        <Bar dataKey="actual" fill="#f39c12" name="Actual Departures" radius={[4, 4, 0, 0]} />
                        <Bar dataKey="predicted" fill="#9b59b6" name="Predicted Departures" radius={[4, 4, 0, 0]} />
                    </BarChart>
                ) : (
                    <LineChart
                        data={departureData}
