...
const RoomSoldGraph: React.FC<RoomSoldGraphProps> = React.memo(({ hotelId }) => {
    const dispatch = useDispatch<AppDispatch>();
    const [timePeriod, setTimePeriod] = useState<'1w' | '1m' | '3m' | '6m' | '12m'>('1m');
    const roomSoldRecords = useSelector((state: RootState) => state.records.roomSold);
    const roomSoldForecasts = useSelector((state: RootState) => state.forecast.roomSold);
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                    <h3 className="text-lg font-bold text-gray-900 flex items-center">
                        <span className="mr-2">🛏️</span> Rooms Sold Analysis
                    </h3>
                ...
                <ResponsiveContainer width="100%" height={chartHeight}>
                    {chartType === 'bar' ? (
                        <BarChart data={roomSoldData} margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
                            ...
                            <Bar dataKey="actual" fill="#ec4899" name="Actual Rooms" radius={[4, 4, 0, 0]} />
                            <Bar dataKey="predicted" fill="#f472b6" name="Predicted Rooms" radius={[4, 4, 0, 0]} />
                        </BarChart>
                    ) : (
